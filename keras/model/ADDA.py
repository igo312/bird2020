...
import numpy as np
import argparse
save_path = r'H:\dataset\BirdClef\paper_dataset\Checkpoint'

class adda():
    def __init__(self, lr, clf_mode, label_len, Img_size):
        # Input shape
    ...
    def define_source_encoder(self, weights=None):
        ...
        x = self.BN_CONV(inp, 64, 7, strides=(2, 2))
        x = MaxPooling2D()(x)
        x = self.BN_CONV(x, 128, 5)
        x = MaxPooling2D()(x)
        x = self.BN_CONV(x, 256, 3)
        x = MaxPooling2D()(x)
        x = self.BN_CONV(x, 512, 3)
        x = MaxPooling2D()(x)
        x = self.BN_CONV(x, 1024, 3)
        x = MaxPooling2D()(x)
    ...
    def get_total_classifer(self, model, model_weights=None):
        '''
        :param model:
        :param model_weights: It is a list including two element which is loss weight ,model weight and mode
        :return:
        '''
        clf_list = []
        weight_list = []
        for weight in model_weights:
            clf_list.append(self.get_source_classifier(model, weight[1], weight[2]))
            weight_list.append(Lambda(lambda x: x*weight[0]))

        output = Add()([weight_list[i](clf_list[i]) for i in range(len(clf_list))])
        return output
    ...
    def _define_discriminator(self, shape):
        ...
        x = Dense(512, activation=LeakyReLU(alpha=0.3), kernel_regularizer=regularizers.l2(0.01),
    ...
    def train_source_model(self, source_gen, val_gen, model, epochs, steps_per_epoch, validation_steps, name,
        ...
        saver = keras.callbacks.ModelCheckpoint(os.path.join(save_path, name+'{epoch:02d}-{val_loss:.02f}.hdf5'),
                                                monitor='val_loss',
                                                verbose=1,
        ...
        scheduler = keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.75, patience=10, verbose=0,
                                                      mode='min')
    ...
    def train_target_discriminator(self, source_gen, target_gen, source_model=None, src_discriminator=None,
                                   tgt_discriminator=None, epochs=50,
                                   save_interval=1, start_epoch=0, num_batches=200):
        ...
        # TODO(11/12):更改路径
        callback1 = keras.callbacks.TensorBoard(os.path.join(save_path,'tensorboard'))
        callback1.set_model(source_discriminator)
        callback2 = keras.callbacks.TensorBoard(os.path.join(save_path,'tensorboard'))
